...
namespace RSAchievementTracker.Domain
{
    public class InitializeUser
    {
        ...
        public InitializeUser(string username)
        {
            // initializes the levels variable to default
            CurrentUser = new User();

            string userSkillsLevel = GetUserInfo(URLSTATS + username);
            string userQuests = GetUserInfo(URLQUESTS + username);

            PopulateStatsData(userSkillsLevel);
            if (userQuests != INVALIDQUESTS)
                PopulateQuestsData(userQuests);
        }
        ...
        private void PopulateStatsData(string statsString)
        {
        ...
        private void PopulateQuestsData(string questsJson)
        {
